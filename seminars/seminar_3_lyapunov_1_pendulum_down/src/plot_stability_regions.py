...
def plot_stability_regions(save_path=None, show_plot=True, figsize=(10, 8), dpi=300):
    ...
    ax.text(0.9, -2.7, "7: Saddle-node\nbifurcation\n$k_1 = 1, k_2 < 0$", ha='center', rotation=90)
    
    # Добавляем метку для региона 8 (вырожденный случай) и стрелку на линию k1=1
    ax.text(0.8, 0.7, "8: Degenerate case\n(one eigenvalue zero)\n$k_1 = 1, k_2 \\geq 0$", ha='center', fontsize=9)
    ax.annotate('', xy=(1.0, 0.3), xytext=(0.8, 0.5),
    ...
    legend_elements = [
        ...
        Line2D([0], [0], color='purple', lw=10, label='7: Saddle-node: $k_1 = 1, k_2 < 0$'),
        Line2D([0], [0], color='cyan', lw=10, label='8: Degenerate case: $k_1 = 1, k_2 \\geq 0$')
    ]
